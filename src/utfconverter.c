...
uint16_t *utf8_to_utf16(const uint8_t *utf8_buffer, size_t buffer_len)
{
    ...
    int utf16_buffer_len = MultiByteToWideChar(CP_UTF8, 0, (const char*)utf8_buffer, len_param, nullptr, 0);

    uint16_t *utf16_buffer = calloc(utf16_buffer_len + 1, sizeof(wchar_t));
    if (utf16_buffer == nullptr) return nullptr;

    int result = MultiByteToWideChar(CP_UTF8, 0, (const char*)utf8_buffer, len_param, utf16_buffer, utf16_buffer_len + 1);
    if (result == 0) {
...
{
    const uint16_t word = 0x00FF;

    const char *bytes = (const char*)&word;
...
uint16_t *utf8_to_utf16(const uint8_t *utf8_buffer, size_t buffer_len)
{
    ...
    const char *utf16name = is_little_endian() ? "UTF-16LE" : "UTF-16BE";
    const char *input_buffer = (const char*)utf8_buffer;
    char *output = iconv_convert("UTF-8", utf16name, input_buffer, buffer_len, 2 * buffer_len, 1);
...
uint8_t *utf16_to_utf8(const uint16_t *utf16_buffer, size_t buffer_len)
{
    ...
    char *output = iconv_convert(utf16name, "UTF-8", input_buffer, buffer_len * 2, buffer_len + buffer_len / 2 + 4, 2);
    if (output == nullptr) {
        output = iconv_convert(utf16name, "UTF-8", input_buffer, buffer_len * 2, 2 * buffer_len, 2);
    }
